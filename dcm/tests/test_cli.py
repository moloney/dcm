...
def test_query(make_local_node, make_dcmtk_nodes, make_dcm_config_file):
    ...
    runner = CliRunner()
    config_path = make_dcm_config_file()
    args = ['--config', config_path, 'query', '--assume-yes', '--level', 'image', '--local', str(local_node), str(remote)]
    print(args)
    result = runner.invoke(cli, args)
    assert result.exit_code == 0
...
def test_sync(make_local_node, make_dcmtk_nodes, make_dcm_config_file):
    ...
    runner = CliRunner()
    config_path = make_dcm_config_file()
    args = ['--config', config_path, 'sync', '--local', str(local_node), '--source', str(src_remote), str(dest_remote)]
    print(args)
    result = runner.invoke(cli, args)
    assert result.exit_code == 0
...
def _run_forward(config_path, local_node, dest_dir):
    runner = CliRunner()
    fwd_args = ['--config', config_path, 'forward', '--inactive-timeout', '10', '--local', local_node, dest_dir]
    return runner.invoke(cli, fwd_args)


@has_dcmtk
def test_forward(make_local_node, make_dcmtk_nodes, make_dcm_config_file, make_local_dir):
    local_node = make_local_node()
    ...
    with ThreadPoolExecutor(max_workers=1) as executor:
        fwd_future = executor.submit(_run_forward, config_path, str(local_node), str(dest_dir))
        time.sleep(1)
        if fwd_future.done():
        ...
        runner = CliRunner()
        sync_args = ['--config', config_path, 'sync', '--local', str(local_node), '--force-all', '--method', 'REMOTE_COPY', '--source', str(src_remote), str(local_node)]
        print(sync_args)
