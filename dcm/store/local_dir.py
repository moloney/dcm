...
def _dir_crawl_worker(res_q, root_path, recurse=True, file_ext='dcm',
    ...
    print('iglob arg: %s' % os.path.join(root_path, glob_exp))
    for path in iglob(os.path.join(root_path, glob_exp), recursive=recurse):
        print("found a path..")
        curr_files.append(path)
        if len(curr_files) == max_chunk:
            print("Putting a chunk...")
            res_q.put(LocalDataChunk(curr_files))
            curr_files = []
    if len(curr_files) != 0:
        print("Putting last chunk")
        res_q.put(LocalDataChunk(curr_files))
